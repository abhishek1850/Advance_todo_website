...
function getSmartGreeting(name: string): string {
    ...
    if (hour < 6) return `🌙 Burning the midnight oil, ${firstName}?`;
    if (hour < 9) return `🌅 Early bird ${firstName}! Best time to focus.`;
    if (hour < 12) return `☀️ Good morning, ${firstName}! Let's be productive.`;
    if (hour < 14) return `🌤️ Afternoon focus time, ${firstName}!`;
    if (hour < 17) return `⚡ Power through, ${firstName}!`;
    if (hour < 20) return `🌆 Evening push, ${firstName}!`;
    return `🌙 Still going strong, ${firstName}!`;
}
